/*
  acidtocid.c -- convert ACID files to CID files (main routine)
...
main(int argc, char **argv)
{
	...
	if (!outfile)
	    thisout = substext(basename(argv[optind]), ".acid", ".cid");
	else
	    thisout = outfile;

	if (cid_write(thisout, cid))
	    ret = 1;
